<?php
    ...
    if(isset($_POST['edit_button'])) {
        ...
        //validation logic
        if (empty($admin_name)) {
            $name_err = "Admin Email Address is required";
        ...
        } elseif (!preg_match("/^\d+\s*,\s*\w+\s+\w+,\s*\w+$/", $hospital_address)) {
            $Hosp_address_err = "Sorry, the provided address format is invalid. Please enter a valid address in the format: '115, Last Lane, NYC'";
        }
...
function update_admin($conn, $email, $name,$hospName, $hospAddr, $hospCont, $hospLogo, $adm_id) {
    ...
    mysqli_stmt_bind_param($update_stmt, "ssssssi", $email,$name,$hospName,$hospAddr,$hospCont,$hospLogo,$adm_id);
